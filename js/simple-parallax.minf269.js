...
(function($) {
    ...
    function legento_simple_parallax() {
        // Only apply parallax if window width is greater than 992px
        if (window.innerWidth > 992) {
            const handleParallaxByClass = (cls, optionsCallback) => {
                const elements = document.querySelectorAll('.' + cls);
                elements.forEach(el => {
                    // Check if the element is already initialized to avoid duplicates
                    if (!el.classList.contains('simple-parallax-initialized')) {
                        const scale = el.dataset.scale ? parseFloat(el.dataset.scale) : 1.05;
                        const transition = el.dataset.transition || 'cubic-bezier(0,0,0,0.5)';
                        new simpleParallax(el, optionsCallback(scale, transition));
                        el.classList.add('simple-parallax-initialized');
                    }
                });
            };

            // Initialize parallax for each style
            handleParallaxByClass('parallax1', (scale, transition) => ({
                orientation: 'right',
                delay: 0.6,
                scale: scale,
            ...
            handleParallaxByClass('parallax4', (scale, transition) => ({
                orientation: 'left',
                delay: 0.6,
                scale: scale,
                transition: transition
            ...
            const parallax2Elements = document.getElementsByClassName('parallax2');
            Array.from(parallax2Elements).forEach(el => {
                if (!el.classList.contains('simple-parallax-initialized')) {
            ...
            const parallax3Elements = document.getElementsByClassName('parallax3');
            Array.from(parallax3Elements).forEach(el => {
                if (!el.classList.contains('simple-parallax-initialized')) {
                    new simpleParallax(el, {
                        delay: 2
                    });
            ...
            const parallax5Elements = document.getElementsByClassName('parallax5');
            Array.from(parallax5Elements).forEach(el => {
                if (!el.classList.contains('simple-parallax-initialized')) {
                    new simpleParallax(el, {
                        overflow: true,
                        delay: 0.6,
    ...
    function handleParallax() {
        // Run parallax initialization
        legento_simple_parallax();

        // In the editor, reinitialize when content changes
        if (typeof wp !== 'undefined' && wp.blocks) {
            const observer = new MutationObserver(() => {
                legento_simple_parallax();
            });

            // Observe changes in the editor's content area
            const editorContent = document.querySelector('.block-editor-block-list__layout') || document.body;
    ...
    // For Gutenberg editor, reinitialize when blocks are updated
    if (typeof wp !== 'undefined' && wp.blocks) {
        wp.domReady(() => {
            handleParallax();
        });
